...
-- CORRECTED VERSION - skips tables that don't exist
...
-- Add proper RLS policy for resources table if missing AND if table exists
DO $$
BEGIN
...
-- Add proper RLS policy for resources INSERT if table exists
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'resources') THEN
    IF NOT EXISTS (
