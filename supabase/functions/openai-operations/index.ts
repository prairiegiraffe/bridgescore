...
/**
 * Score a call using client's assistant
 */
async function scoreCall(
  { transcript, clientId }: { transcript: string; clientId: string },
  openaiHeaders: Record<string, string>,
  supabaseClient: any
) {
  ...
  const bridgeSteps = client.bridge_steps as any[]
  const stepScores = []
  ...
  return {
    ...
    stepScores,
    clientId,
    assistantId: client.openai_assistant_id,
